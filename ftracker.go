...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	runend := ((runningCaloriesMeanSpeedMultiplier * speed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return runend
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	meanSpeedSquare := math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)
	walkend := ((walkingCaloriesWeightMultiplier*weight + (meanSpeedSquare/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return walkend
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	swimend := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimend
}
